...
public class AnalyseTexte {
    ...
    public static Set<String> obtenirExpressionsCles(String texte) throws IOException {
        ...
        for (String token : tokens) {
            if (token.length() > limiteParDoc)
                throw new RuntimeException(
                        " (AnalyseTexte) Un token à lui seul dépasse la limite de taille");
        ...
        byte[] encodedText =
                new JSONObjectUneCle("documents", documents).toString().getBytes("UTF-8");
        ...
        String corpsRep =
                CharStreams.toString(new InputStreamReader(connexion.getInputStream(), "UTF-8"));
