        ...
        try {
            ...
            if (etape == 1) { // Première étape de la connexion
                ...
                if (!resultat.isNull(Authentification.CLE_ERREUR)) {
                    codeHttp = resultat.get(Authentification.CLE_ERREUR).equals(Authentification.ERR_INTERNE)
                            ? HttpStatus.INTERNAL_SERVER_ERROR
                            : HttpStatus.CONFLICT;
                } else {
                    codeHttp = HttpStatus.OK;
                    corpsReponse.put(Authentification.CLE_ENVOI_CODE,
                ...
                if (!resultat.isNull(Authentification.CLE_ERREUR))
                    ...
                    final EntiteUtilisateur entiteU = EntiteUtilisateur.obtenirEntiteOuCreer(id, logger);
