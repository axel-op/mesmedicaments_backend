...
public abstract class AbstractEntite extends TableServiceEntity {
	...
	protected static <E extends AbstractEntite> Optional<E> obtenirEntite(String table, String partition, String rowKey,
			Class<E> clazzType) throws StorageException, URISyntaxException, InvalidKeyException {
		TableOperation tableOperation = TableOperation.retrieve(partition, rowKey, clazzType);
		return Optional.ofNullable(obtenirCloudTable(table).execute(tableOperation).getResultAsType());
	}

	protected static <E extends AbstractEntite> Iterable<E> obtenirToutesLesEntites(String table, String partition,
			Class<E> clazzType) throws StorageException, URISyntaxException, InvalidKeyException {
		String filtrePK = TableQuery.generateFilterCondition("PartitionKey", QueryComparisons.EQUAL, partition);
		return obtenirCloudTable(table).execute(new TableQuery<>(clazzType).where(filtrePK));
	}
	...
	public void creerEntite() throws StorageException, URISyntaxException, InvalidKeyException, RuntimeException {
		checkConditions();
		obtenirCloudTable(table).execute(TableOperation.insertOrReplace(this));
	}

	public void mettreAJourEntite() throws StorageException, URISyntaxException, InvalidKeyException, RuntimeException {
		checkConditions();
		obtenirCloudTable(table).execute(TableOperation.insertOrMerge(this));
	}
	...
	/**
	 * Doit renvoyer false s'il y a un problème avec l'entité. Une exception sera
	 * alors levée avant la mise à jour serveur, sauf en cas de suppression.
	 * 
	 * @return booléen
	 */
	...
	public String toString() {
		return "\tClasse = " + this.getClass().toGenericString() + "\tTable = " + this.table + "\tPartitionKey = "
				+ this.getPartitionKey() + "\tRowKey = " + this.getRowKey();
	}

	@Override
	public boolean equals(Object o) {
		if (!(o instanceof AbstractEntite))
			return false;
		AbstractEntite other = (AbstractEntite) o;
		return (other.getPartitionKey().equals(this.getPartitionKey())) && (other.getRowKey().equals(this.getRowKey()));
	}
