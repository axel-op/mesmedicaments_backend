...
public final class MiseAJourInteractions {
	...
	private static final String[] regexRisques = new String[] { REGEX_RISQUE1, REGEX_RISQUE2, REGEX_RISQUE3,
			REGEX_RISQUE4 };
		...
		if (executionEnCours.compareAndSet(false, true)) {
			...
			if (nomsSubImporteesNormalises.isEmpty()) {
				MiseAJourClassesSubstances.importerSubstances(logger).entrySet().stream()
						.map(e -> new AbstractMap.SimpleEntry<>(Texte.normaliser.apply(e.getKey()), e.getValue()))
						.forEach(e -> nomsSubImporteesNormalises.computeIfAbsent(e.getKey(), k -> new HashSet<>())
								.addAll(e.getValue()));
			}
			nouveauxChamps();
			try {
	...
	private static PDDocument recupererFichier() throws IOException {
		...
		long startTime = System.currentTimeMillis();
		HttpsURLConnection connexion = (HttpsURLConnection) new URL(URL_FICHIER_INTERACTIONS).openConnection();
		...
		if (ignorerLigne)
			...
			if (ligne.matches("(?i:thesaurus .*)"))
				...
				if (taille.equals(TAILLE_NOM_SUBSTANCE)) {
					...
					if (substance1EnCours != null) {
						...
						if (!texte.equals("")) {
							...
							if (Float.compare(xposition, colonneGauche) <= 0) {
								if (valeurInconnueDesc == null)
									valeurInconnueDesc = matrice[2][1];
								...
								if (valeurInconnueCond == null)
									valeurInconnueCond = matrice[2][1];
						...
						for (int i = d; i < substances.length; i++) {
							...
							if (!substance.matches(" *")) {
								if (i == substances.length - 1 && ajoutSubstances != null) {
									...
									substances2EnCours.add(substance);
							}
						}
					}
				} else {
					...
					logger.warning(
							"LIGNE IGNOREE (il ne devrait pas y en avoir) : " + Utils.NEWLINE + "\"" + texte + "\"");
				}
			}
		}
	}
	...
	private static void nouveauxChamps() {
		if (classeEnCours != null) {
			classesSubstances.put(Texte.normaliser.apply(classeEnCours).toLowerCase(),
	...
	private static void ajouterInteractionsBrouillon() {
		if (risqueEnCours != null) {
			for (String substance2 : substances2EnCours) {
				interactionsBrouillon.add(new InteractionBrouillon(substance1EnCours, substance2, risqueEnCours,
	...
	private static void creerEntitesInteraction() {
		...
		for (InteractionBrouillon intBrouillon : interactionsBrouillon) {
			...
			if (intBrouillon.descriptif.matches(" *")) {
				...
				conduite = Texte.corrigerApostrophes
						.apply(intBrouillon.conduite.replaceAll("((CI ?)" + "|(ASDEC ?)" + "|(PE )" + "|(APEC ?))", "")
								.replaceFirst(" ?(- )+\n", ""));
			}
	...
	private static class Recherche {
		...
		protected static Function<String, Set<EntiteSubstance>> obtenirCorrespondances = recherche -> cacheCorrespondances
				.computeIfAbsent(recherche, exp -> {
					if (exp == null)
						return new HashSet<>();
					exp = exp.toLowerCase();
					...
					if (classesSubstances.containsKey(exp)) {
						return classesSubstances.get(exp).stream()
								.flatMap(substance -> Recherche.obtenirCorrespondances.apply(substance).stream())
		...
		private static Set<EntiteSubstance> rechercherMeilleuresSubstances(String recherche) {
			return cacheMeilleuresSubstances.computeIfAbsent(recherche, terme -> {
				...
				if (terme.matches(".+\\(.+\\)")) {
					...
					if (resultats1.isEmpty())
						return resultats1;
					return rechercherMeilleuresSubstances(terme.replaceFirst(debut, "")).stream()
							.filter(resultats1::contains).collect(Collectors.toSet());
				}
				String regexExclus = "(?i:" + "(fruit)" + "|(acide)" + "|(alpha))" + "|(?i:.*par voie.*)";
				for (String expression : (Iterable<String>) () -> obtenirSousExpressions(
						terme.trim().replaceAll("[,\\(\\)]", "")).stream().map(Texte.normaliser::apply)
								.map(String::toLowerCase).filter(exp -> !exp.matches(regexExclus))
								.filter(exp -> exp.equals("fer") || !exp.matches("([^ ]{1,3} ?\\b)+")).iterator()) {
					if (expression.matches("(?i:(sauf)|(hors))"))
						break;
		...
		private static Set<EntiteSubstance> rechercheSimple(String recherche) {
			return cacheRechercheSubstances.computeIfAbsent(recherche, mot -> {
				Set<EntiteSubstance> matchPrecis = nomsSubImporteesNormalises.entrySet().stream()
						.filter(e -> e.getKey().matches("(?i:.*" + mot + "\\b.*)")).flatMap(e -> e.getValue().stream())
						.collect(Collectors.toSet());
				if (!matchPrecis.isEmpty())
					return matchPrecis;
				return nomsSubImporteesNormalises.entrySet().stream()
						.filter(e -> e.getKey().matches("(?i:.*" + mot + ".*)")).flatMap(e -> e.getValue().stream())
						.collect(Collectors.toSet());
			});
		}
	}
	...
	private static <T> Set<T> trouverMeilleurs(HashMap<T, Double> classement) {
		...
		return classement.keySet().stream().filter(cle -> classement.get(cle) == scoremax).collect(Collectors.toSet());
	}
	...
	private static class Texte {
		...
		protected static Function<String, String> normaliser = original -> cacheNormalisation.computeIfAbsent(original,
				mot -> {
		...
		protected static Function<String, String> corrigerApostrophes = original -> cacheApostrophes
				.computeIfAbsent(original, mot -> {
					byte[] ancien = mot.getBytes(CHARSET_1252);
					byte[] nouveau = new byte[ancien.length];
					for (int i = 0; i < ancien.length; i++) {
						switch (Integer.valueOf(ancien[i])) {
					...
					return new String(nouveau, CHARSET_1252).trim().replaceAll("  ", " ");
				});
	}
}