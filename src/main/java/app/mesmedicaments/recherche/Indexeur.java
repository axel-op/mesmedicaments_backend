...
public final class Indexeur {
    ...
    private JSONObject preparerLesDocuments() {
        ...
        logger.info(
                String.valueOf(documents.size())
                        + " documents prêts à être envoyés pour l'indexation");
    ...
    private JSONObject medicamentEnDocument(AbstractEntiteMedicament<?> medicament) {
        ...
        final Set<JSONObject> substances =
                obtenirSubstances(medicament).stream()
    ...
    private Set<EntiteSubstance> obtenirSubstances(AbstractEntiteMedicament<?> medicament) {
        final Set<Long> codes =
                medicament.getSubstancesActivesSet().stream()
                        .map(s -> s.codeSubstance)
                        .collect(Collectors.toSet());
        return codes.parallelStream()
                .map(
                        code ->
                                cacheSubstances.computeIfAbsent(
                                        code,
                                        Unchecker.wrap(
                                                logger,
                                                (Long k) ->
                                                        EntiteSubstance.obtenirEntite(
                                                                        medicament.getPays(), k)
                                                                .get())))
                .collect(Collectors.toSet());
    }
